...
export default function Footer({ locale }: FooterProps) {
	...
	const footer = isArabic
		? {
				about: "أفورا تساعدك على إدارة أعمالك وفريقك من مكان واحد.",
				services: "خدماتنا",
				serviceLinks: ["إدارة الفريق", "إدارة المشاريع", "التقارير", "الأمان"],
				sections: "الأقسام",
				sectionLinks: ["الرئيسية", "من نحن", "الأسعار", "تواصل معنا"],
				branches: "الفروع",
				branchLinks: ["القاهرة", "الرياض", "دبي", "لندن"],
				newsletter: "النشرة الإخبارية",
				newsletterText: "اشترك لتصلك آخر الأخبار والتحديثات.",
			...
			<div className="mx-auto w-full max-w-6xl px-4 py-14 sm:px-6 lg:py-16">
				<div className="grid gap-12 lg:grid-cols-[1.3fr_2fr_1.3fr]">
					...
					<div className="grid grid-cols-2 gap-x-8 gap-y-10 sm:grid-cols-3">
						...
						<FooterColumn title={footer.sections} links={footer.sectionLinks} locale={locale} />
				...
				<div className="mt-14 flex flex-col gap-3 border-t border-background/15 pt-6 text-xs text-background/50 sm:flex-row sm:items-center sm:justify-between">
					...
					<span>{isArabic ? "جميع الحقوق محفوظة" : "All rights reserved"}</span>
				</div>
			</div>
		</footer>
	);
